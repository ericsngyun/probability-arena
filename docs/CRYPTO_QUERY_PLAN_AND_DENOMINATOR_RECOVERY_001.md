...
| B2 verdict | **zero regressions.** 25 hot paths measured; 5 changed plan, all improved or latency-neutral; every probe that got slower had a **byte-identical plan**, i.e. noise |
| B3 verdict | **PASS.** 0 lock failures in every arm. Realistic-arrival writer wait p95 **0.53 s → 0.018 s**, max **1.53 s → 0.078 s** |
| B4 arrivals | p95 **425/day** (recent 14 complete days), planning rate 530/day |
| B4 safe capacity | **120 → 1 280 tokens/pass** measured under a live competing writer; **480 → 5 120/day**; deficit (0.91×) → **9.7× margin** |
| B5 backlog | 11 918 tokens: **78.1 % RETENTION_LOST**, 13.3 % MISSING_REQUIRED_INITIAL_STATE, 6.6 % PARTIALLY_RECOVERABLE, 1.6 % no-evidence, **0.39 % (47 tokens) RECOVERABLE_NOW** |
| B5 the real finding | the observation lane's median **last** tick is **83 minutes** after birth; in a cohort where **nothing has been pruned**, 24 h tick coverage is **4.1 %**. The 24 h evidence is not being deleted — it was never recorded |
...
| B8 | live `ANALYZE` **EXECUTED** under gate: **0.4597 s**, 130 stat rows, **Δ file bytes 0**, `integrity_check ok`, **0 new lock events**, Alembic still 0027 |
...
first row is produced, so the four numbers above with a temp B-tree are exact.
...
|---|---|---|---|---|
| **V0** none at all | `ix_…_chain` — 65.80 ms | `ix_…_chain` — 40.52 ms | `ix_…_chain` — 13.46 ms | `ix_…_chain` — 3.18 ms |
| **V1** the table row only (`idx IS NULL`) | `ix_…_chain` — 64.96 ms | `ix_…_chain` — 39.97 ms | `ix_…_chain` — 13.03 ms | `ix_…_chain` — 2.32 ms |
| **V2** the **bad** index's row only (`ix_…_chain`) | **`ix_…_token_address` — 0.039 ms** | **`ix_…_token_address` — 0.023 ms** | **`ix_…_token_address` — 0.006 ms** | **`ix_…_base_token_address` — 0.010 ms** |
| **V3** the **good** index's row only (`ix_…_token_address`) | `ix_…_chain` — 65.16 ms | `ix_…_chain` — 39.99 ms | `ix_…_chain` — 13.37 ms | **`ix_…_base_token_address` — 0.009 ms** |
| **V4** both index rows | `ix_…_token_address` — 0.038 ms | `ix_…_token_address` — 0.023 ms | `ix_…_token_address` — 0.006 ms | `ix_…_base_token_address` — 0.009 ms |
| **V5** full `ANALYZE` of this table | 0.038 ms | 0.023 ms | 0.005 ms | 0.009 ms |
| **V6** full `ANALYZE` of everything | 0.037 ms | 0.022 ms | 0.005 ms | 0.009 ms |
...
The table row (`idx IS NULL`, the row-count estimate) does nothing on its own
(V1). This is not "ANALYZE helps"; it is one specific row per table, and the
same defect exists anywhere a low-cardinality column carries its own index —
which is why B2 found the identical pattern on `crypto_opportunity_signals`.
...
planner swaps to the two-column one and pays for the wider index. This is a
real +12.5 ms per pass. It is **1 pass in 4 per day**, against a per-token path
...
The mechanism harness attributes it precisely. On the selection query:

| variant | plan for `crypto_tokens` | median |
|---|---|---|
| V0 no statistics | `ix_crypto_tokens_chain` | 11.82 ms |
| V2 `ix_crypto_tokens_chain` row only | `ix_crypto_tokens_chain_address` | **23.30 ms** |
| V2b `ix_crypto_tokens_chain_address` row only | `ix_crypto_tokens_chain` | 10.02 ms |
| V4 both `crypto_tokens` index rows | `SCAN crypto_tokens` | **9.61 ms** |
| V6 full `ANALYZE` (adds the outcome-table statistics) | `ix_crypto_tokens_chain_address` | **22.63 ms** |

So the +12 ms is caused by the `ix_crypto_tokens_chain` statistic **in
combination with** the `crypto_token_survival_outcomes` statistics that change
the outer-join costing: with only the `crypto_tokens` rows present, SQLite picks
the best plan of all (`SCAN`, 9.61 ms). That is a genuinely useful pointer for
whoever revisits this selection query — the fix is not a new index, it is that
`chain=?` should not be in the predicate at all on a single-chain deployment.
...
| `tick_agg.report_build` | 4 148.6 | 4 151.6 | 4 225.1 | +1.8 % | 1/11 | IMPROVED (see below) |
...
because those four probes looked slower at n=2 with an unchanged plan. All four
returned to (or below) their before values. That is the whole content of the
"+29.6 %" that `calibration.summary` showed at n=2: I/O jitter on a 4.5 GB file,
not a planner decision.

### 2.1 The five plans that actually changed
...
and it was **not previously known**: it is on the crypto scan path, runs once
per token per scan, and was paying 15 ms per call to walk a 90 356-row index
whose every entry is `'solana'`. `ANALYZE` fixes it as a side effect.
...
lock event on this host** (§9.3), and `ANALYZE` cuts the read half of its
sub-window loop by 4.9× and 11.8×.

### 2.2 B2 verdict

**No material production hot path regresses.** Twenty of twenty-five probes
have a byte-identical plan; of the five that change, three improve by 4.9×–2 176×
and two move by under 0.2 ms on statements that run once per report. The gate
that protects everything else is **GREEN**.
...
|---|---|---|
| tokens processed / pass | 120 / 125 / 125 / 125 / 135 | **1 280 / 1 285 / 1 300 / 1 300 / 1 300** |
| pass wall | 20.03–20.46 s | 20.02–20.08 s |
| reconciler txn hold p50 | 0.550–0.578 s | **0.0204–0.0212 s** |
| reconciler txn hold p95 | 0.579–0.707 s | **0.0287–0.0334 s** |
| reconciler txn hold max | 0.642–**2.025 s** | 0.103–**0.261 s** |
| competing-writer wait p50 | 0.054–0.329 s | **0.0001 s** |
| competing-writer wait p95 | 0.329–0.629 s | **0.0183–0.0184 s** |
| competing-writer wait **max** | **1.530 s** | **0.0785 s** |
| `database is locked` failures | **0** | **0** |
| retries | **0** | **0** |
| competitor successful writes | 16–18 per 20 s | 19–22 per 20 s |
| `ANALYZE` build time | — | 0.464–0.495 s |

**Both arms pass the criteria; the after arm passes by 1–2 orders of
magnitude.** Wait p95 improves **29–34×** and worst-case wait **19.5×**, while
throughput improves **10.4×** — the two move in the same direction, which is
the thing that could not be assumed and had to be measured.

The numbers are internally consistent, which is worth stating because it is the
check that would have caught a broken harness: before, the reconciler holds the
lock 0.55 s × ~27 batches ≈ 15 s of a 20 s pass (75 % duty), so most random
arrivals collide and the median wait is a substantial fraction of a hold; after,
it holds 0.020 s × ~257 batches ≈ 5.2 s of 20 s (26 % duty), so most arrivals
do not collide at all and the median wait collapses to 0.1 ms.

**Note on hold max.** One before-arm trial recorded a 2.025 s hold — slightly
**over** the 2.0 s `RECONCILE_WRITE_TIME_SLO_SECONDS`. The after arm's worst
hold across five trials is 0.261 s, **13 % of the SLO**. The read fix does not
merely raise throughput; it removes the only observed SLO breach.
...
realistic arm, both before and after. Competing-writer wait p95 **0.53 s →
0.018 s** and max **1.53 s → 0.078 s**, both far inside the 1.0 s / 5.0 s
envelope declared before the runs. Throughput and writer-friendliness improved
together, which was the open question — the prior work explicitly recorded
that hold and wait are different metrics and could not be assumed to move
proportionally. Measured here, directly, in a second process: they did.

---

## 4. B4 — arrivals and safe capacity, re-measured

Measured 2026-08-11 07:00 UTC from `crypto_tokens.first_seen_at`, chain
...
The task's baseline is confirmed (524/454/421/396 vs 520/454/421/396) and so is
...
Per UTC calendar day, complete days only (37 days, 2026-07-04 → 2026-08-10):
...
| | before `ANALYZE` | after `ANALYZE` |
|---|---|---|
| tokens/pass, measured minimum across 5 trials | 120 | **1 280** |
| tokens/pass, measured median | 125 | 1 300 |
| **safe tokens/pass (conservative = measured min)** | **120** | **1 280** |
| safe tokens/hour (amortised over the 6 h interval) | 20 | **213** |
| **safe tokens/day (× 4 passes)** | **480** | **5 120** |
| vs p95 arrivals 425/day | **0.88× — DEFICIT** | **12.0×** |
| vs planning arrivals 530/day | **0.91× — DEFICIT** | **9.7×** |
| write-lock hold p95 as share of the 2.0 s SLO | 29–35 % (one trial **breached** at 2.025 s) | **1.4–1.7 %** |

Before the fix the reconciler could not even service arrivals, let alone drain
backlog — `INSUFFICIENT_RECONCILIATION_CAPACITY` was the correct verdict. After
the fix there is **9.7× margin over the planning arrival rate**, i.e. 4 590
tokens/day of excess available for backlog drain.
...
| birth events | 7 436 |
| **backlog size** | **11 918** |
| oldest backlog `first_seen_at` | 2026-07-04 01:36 UTC (38.2 days) |
| backlog tokens with any retained tick at all | 2 255 (18.9 %) |

### 5.1 The partition

| class | tokens | share | what a pass can do with it |
|---|---|---|---|
| **RETENTION_LOST** | **9 306** | **78.08 %** | write it off — the latest tick that could still have qualified is older than `crypto_retention_days` and `retention.py` deleted it |
| **MISSING_REQUIRED_INITIAL_STATE** | **1 581** | **13.27 %** | nothing, ever — no `initial_liquidity_usd` to compare against (or no first-evidence anchor), so every survival label is structurally `None` and the token can never reach `final` until it decays into RETENTION_LOST |
| **PARTIALLY_RECOVERABLE** | **789** | **6.62 %** | real 15 m / 1 h / 6 h labels; the 24 h answer is permanently unavailable |
| **NO_EVIDENCE_IN_ANY_HORIZON** (typed "other") | **195** | **1.64 %** | nothing — anchored, due, inside retention, but no tick lands in any horizon window |
...
| **sum** | **11 918** | 100 % | reconciles to the backlog size exactly |

**The prior estimate was wrong in the direction that matters.** It said ~79.6 %
retention-lost (confirmed: 78.08 %) and "only ~2 400 potentially recoverable".
2 612 tokens are indeed not retention-lost — but of those, **47** can produce a
...
| RECOVERABLE_NOW / PARTIALLY_RECOVERABLE / NO_EVIDENCE_IN_ANY_HORIZON | cannot be separated without touching `crypto_price_ticks` | one `EXISTS` on `ix_crypto_price_ticks_token_address` — **0.006 ms after `ANALYZE`, 13.4 ms before** | no |
...
the query plan was fixed. Before `ANALYZE`, probing 11 918 backlog tokens for
recoverability would have cost 160 s of pure read; after, it costs 0.07 s.

**Recommended selection consequence (proposed, not implemented):** the two
monotone classes — 9 306 + 1 581 = **10 887 tokens, 91.3 % of the backlog** —
can be excluded from selection by two indexable predicates with no source load
at all. That is the difference between a reconciler that spends its budget
re-deriving write-offs and one that spends it on the 1 031 tokens that still
...
|---|---|---|---|---|---|---|
| **36 h – 3 d (zero pruning possible)** | 610 | 603 (98.9 %) | 595 (97.5 %) | **79 (13.0 %)** | **25 (4.1 %)** | 236 (38.7 %) |
| 3 d – 7 d | 1 582 | 1 568 (99.1 %) | 1 539 (97.3 %) | 230 (14.5 %) | 54 (3.4 %) | 622 (39.3 %) |
| 7 d – 8.5 d (24 h window partly pruned) | 610 | 106 (17.4 %) | 123 (20.2 %) | 31 (5.1 %) | 13 (2.1 %) | 249 (40.8 %) |

And the observation span itself — first evidence to the **last** tick this
lane ever recorded for a token, over the 2 345 tokens that have any:

| p50 | p90 | max |
|---|---|---|
| **83.1 minutes** | 326.0 minutes (5.4 h) | 11 267.9 minutes |
...
Coverage therefore collapses from 98.9 % at 15 m to 13.0 % at 6 h to **4.1 % at
24 h — in a cohort where nothing has been pruned**. Separately,
`initial_liquidity_usd` is NULL on 4 446 of 7 436 birth events (59.8 %), so even
a token with a 24 h tick has a ~39 % chance of having a baseline to compare it
against.
...
per day out of 425–530**. Fixing the query plan raises capacity 10.4×; it
cannot raise a denominator that the observation lane never filled.
...
* A token does **not** need to be visited every pass. It needs **one** visit
  ...
  **6.5-day-wide recoverable window**, or 26 passes at the 6-hourly cadence.
...
| symbol | value | source |
|---|---|---|
| `C` safe tokens/pass | **1 280** | B3, conservative minimum of 5 trials with a live competing writer |
| passes/day | **4** | `RECONCILER_CADENCE_HOURS = 6` |
| `C_day` safe tokens/day | **5 120** | 1 280 × 4 |
...
| `L` current backlog | **11 918** | B5 |

**Step 1 — how the pass splits.** Backlog-first with a 1 000-slot reserve and
1 280 tokens actually processed:
```
backlog lane   = min(1000, 1280)            = 1 000 tokens/pass  = 4 000/day
in-window lane = 1280 − 1000                =   280 tokens/pass  = 1 120/day
```
...
visits/day against a standing backlog of 11 918:
```
first full sweep of the existing backlog = 11 918 / 4 000 = 2.98 days
```
After that sweep: 9 306 RETENTION_LOST and 47 RECOVERABLE_NOW have `final = 1`
and leave selection permanently; 2 565 remain non-final and age out on their own
...
**Both conditions hold simultaneously**, with the binding constraint being
step 3 at 6.5 passes of slack. **The answer is not INSUFFICIENT.**

For completeness, the same arithmetic **before** the read fix: `C_day = 480`,
`backlog lane = min(1000, 120) = 120/pass = 480/day` against 132.5 arrivals per
pass. Step 2 gives a margin of **0.91×** — below 1 — so the frontier band width
in step 3 is negative and tokens expire unvisited every single day. That is
...
Capacity is 5 120 visits/day. **Yield is 0.39 %** (B5). The allocation policy
above provably services arrivals and drains backlog — of *write-offs*. Of the
11 918 backlog tokens it will visit in 2.98 days, **47** produce a measurement
and 11 871 produce a label meaning "we could not tell". Sizing the reconciler
...
standing population from 3 445 to roughly 300 and turn a 7.5× margin into a
90× one. That is a selection-predicate change, and it belongs to a design pass,
...
The counterfactual ceiling: how many of today's 11 918 backlog tokens would
...
24 h tick coverage is **4.1 %** and 6 h coverage is **13.0 %**. The observation
...
Quantified: at 4.1 % coverage and 530 arrivals/day, extending 7 d → 14 d
...
|---|---|---|---|
| B2 shows no unresolved material regression | mandatory | 25 hot paths; 20 with byte-identical plans; 5 changed, 3 improved (4.9×–2 176×), 2 neutral (< 0.2 ms). No unresolved regression. | **PASS** |
...
| integrity green | mandatory | `PRAGMA integrity_check` = `ok` on the byte-faithful online copy (4.8 s); `PRAGMA quick_check` = `ok` on the live file | **PASS** |
...
* **The good index's statistic is not what fixes the plan** (§1.3). On the
  ...
  ~10-row default for an un-analysed indexed equality.
* **`ANALYZE` is not uniformly good for the reconciler.** Three of its six
  selection queries got slower (§1.6), by a combined ~+12 ms per pass. The
  ...
  once-per-pass selection 12 ms" is true.
* **The prior estimate that ~2 400 backlog tokens were "potentially
  ...
  2 612 are not retention-lost; **47** can produce a 24 h answer (§5.1).
* **Retention length is not the denominator bottleneck** (§5.3, §7.2). In a
  cohort with zero possible pruning, 24 h coverage is 4.1 %.
...
* **Long-run statistics staleness.** `sqlite_stat1` is a snapshot. Tables grow;
  ...
  single most important follow-up** (§10.1).
...
* **`run_scheduled_reconciliation` end to end.** The harnesses call `run_once`
  ...
  SELECT per pass.
* **Whether the 4.1 % / 13.0 % observation coverage is a scout scheduling
...
* **Copies:** every benchmark ran against `sqlite3.Connection.backup()` copies
  ...
  Four of the five scripts refuse any path containing
  `projects/probability-arena/data`, `/var/lib/` or `/srv/`. The fifth
  (`sqlite_analyze_maintenance.py`) exists to touch the live file and is gated
  as described in §8.2.
