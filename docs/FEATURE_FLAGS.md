...
| `CRYPTO_TAPE_RECONCILER_LIMIT` | 2000 | Selection cap. Must exceed IN-WINDOW ELIGIBLE tokens PLUS the intended backlog budget, not merely "the window" — a limit that only covers the in-window head starves the aged-out backlog of budget PERMANENTLY (`status=partial`, `backlog_processed=0`, every pass — CRYPTO-COVERAGE-REPAIR-001 NEW-BLOCKING-1), not just for one pass. LOWERING this in response to write-contention warnings re-opens that trap; prefer raising `CRYPTO_TAPE_RECONCILER_BATCH_SIZE` or lowering `CRYPTO_TAPE_RECONCILER_MAX_DURATION_SECONDS` instead. A binding limit usually reports `status=partial` (not `status=truncated`), since it is also deadline/contention-stopped at production density; watch `backlog_processed` and `oldest_unreconciled_age_hours` (or `status=backlog_expiring`), not the `truncated` flag alone, to tell a healthy pass from one silently losing labels to retention pruning; CLI override `--limit` |
| `CRYPTO_TAPE_RECONCILER_BATCH_SIZE` | 5 | Tokens committed per write transaction (bounds write-lock hold duration, not a competing writer's worst-case wait — see `docs/EVO_X2_RUNBOOK.md`). COUNT-based, not time-based — not portable across hosts; **pre-activation step: measure a real batch's hold on the target host and set this so the hold stays under the busy_timeout** (CRYPTO-COVERAGE-REPAIR-001 NEW-HIGH-1: the previous default of 25 produced 26.3-36.5s holds at EVO-measured host speed and could not converge against the birth rate); CLI override `--batch-size` |
...
| `CRYPTO_TAPE_RECONCILER_LOCK_WAIT_BUDGET_SECONDS` | unset | CRYPTO-RECONCILER-LOCK-WAIT-BUDGET-001: how long ONE SQLite lock ACQUISITION may WAIT. **Not** the same quantity as the write-hold SLO (`CRYPTO_TAPE_RECONCILER_TIME_BUDGET_SECONDS`), which bounds how long a transaction may HOLD the lock. Unset (the shipped default) derives the budget per attempt from the pass's own remaining `MAX_DURATION_SECONDS` — `max(0.5s, remaining/4)` — so a blocked statement can never push the pass past its deadline (EVO `tape_run_id=3618`: `blocked_ms=45,744`, `duration_ms=61,047` against a 30s deadline). A positive value is a tighter CAP, never a floor; derive it from the `lock_wait_ms` histogram now persisted on every run row, never guess it. CLI override `--lock-wait-budget-seconds` |
| `ENABLE_CRYPTO_SPARSE_OBSERVATION` | false | CRYPTO-COVERAGE-REPAIR-002 (**branch `crypto-coverage-repair-002`, NOT merged, NOT deployed**): gates the scheduled **prospective** sparse observer — at most TWO 6h and two 24h DexScreener observations per eligible new birth, inside a fixed +/-60min band. Off = no enrolment, no fetch, no write. A provider ANSWER (`observed`/`provider_no_pair`/`no_liquidity_state`/`token_inactive`) is terminal; a failed REQUEST is not an answer and is re-plannable once while its band is open. No SolanaTracker (structurally denied by a run-scoped provider policy, not by convention), no cohort arming, no polling, no backfill, no interpolation. Adds ONE standing cohort with ROLLING membership that every frozen-cohort entry point refuses, and ONE additive index (migration 0029). The hourly timer is an operator action and is NOT part of this branch — the write-phase lock numbers are reported in the result but not yet persisted to a run table |
| `CRYPTO_SPARSE_OBSERVATION_ENROL_LIMIT` | 200 | Births admitted to the standing rolling cohort per pass. CHOSEN with a stated margin (~9x) over MEASURED EVO arrivals (392.6/417.3/441.3/517.0 per day at 14d/7d/3d/24h; planning rate ~530 ⇒ ~22 per hourly pass) |
| `CRYPTO_SPARSE_OBSERVATION_OBSERVE_LIMIT` | 100 | Observations **and** DexScreener requests per pass (each birth is observed exactly twice; the 6h/24h bands never overlap ⇒ ~44/pass steady state, ~2.3x headroom). May never exceed the horizon lane's hard `OBSERVE_MAX_CALLS=100`; a larger value is refused (`invalid_observe_limit`) |
| `CRYPTO_SPARSE_OBSERVATION_WRITE_BATCH_SIZE` | 25 | Tokens committed per write transaction. The FETCH phase holds **no** transaction at all (network and writes are separate phases, pinned by test), so this bounds only small network-free INSERT batches — it is not the write-lock safety argument `CRYPTO_TAPE_RECONCILER_BATCH_SIZE` is |
| `CRYPTO_SPARSE_OBSERVATION_MAX_DURATION_SECONDS` | 90.0 | Wall-clock deadline on the FETCH phase. CHOSEN, not measured on the target host (~0.9s/request at the 100-request cap, against DexScreener's documented 300 rpm token endpoint). A pass stopping here reports `status=partial`/`stop_reason=deadline`; the remaining member-horizons stay selectable while their band is open |
