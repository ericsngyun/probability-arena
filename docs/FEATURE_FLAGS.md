...
| `ENABLE_CRYPTO_SPARSE_OBSERVATION` | false | CRYPTO-COVERAGE-REPAIR-002: gates the scheduled **prospective** sparse observer — exactly one 6h and one 24h DexScreener observation per eligible new birth. Off = no enrolment, no fetch, no write. No SolanaTracker (structurally denied by a run-scoped provider policy, not by convention), no cohort arming, no retries, no backfill |
