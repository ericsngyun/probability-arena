...
- **B10 (migration governance) — AUDITED, NOT CLOSED.** Confirmed: `run_migrations()` is called unconditionally at ~100 call sites across `app/cli.py`, including `marketops_run_once` and `watch_loop`; the MarketOps timer runs every 5 minutes on EVO, so any `git pull` gets its migrations auto-applied within 5 minutes, before any operator step or backup. This needs a genuine DEVELOPMENT-vs-PRODUCTION-TIMER behavior split (a new `MIGRATION_REQUIRED` fail-clear runtime mode for the timer paths, distinct from the current auto-upgrade-on-every-command behavior every other CLI entry point relies on for first-install convenience) — an architectural decision, not a bug fix, and touching ~100 call sites without a dedicated design-and-test pass would be exactly the "shotgun edit" this role is supposed to avoid. Left unfixed; flagged for a dedicated milestone.
