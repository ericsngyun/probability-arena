"""STRICT NO-LOOKAHEAD delta-S: our source-backed forecasts vs the contemporaneous executable market.
...
for tk, bs, cb, ca in c.execute(
    ...
    "close_bid, close_ask from market_price_tick_buckets "
...
for tk, fn, p, ca in c.execute(
    ...
    best = None
    j = i - 1                      # STRICT: bucket END must precede the forecast
    if 0 <= j < len(arr):
        best = ((ca - arr[j][0]) * 86400.0, arr[j][1])
    if best is None or best[0] < 0 or best[0] > 900:
        skipped_no_quote += 1
        continue
    rows.append((tk, fn, float(p), ws, min(max(best[1], 0.0), 1.0)))

print(f"matched (source_backed, resolved, quote ENDING within 900s BEFORE forecast): {len(rows)}"
