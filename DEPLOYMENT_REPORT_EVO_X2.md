...
## CRYPTO-HORIZON-ORCHESTRATOR — cohort-5 orphan cleanup (2026-07-16, ~18:1x UTC)

Explicitly approved isolated disarm of the pre-fix CANARY-002 orphaned units. Preview (`crypto-horizon-disarm-cohort --cohort-id 5`) selected **only** `probability-arena-horizon-c5-j1.{service,timer}` (regex-scoped to cohort 5). Failure evidence confirmed durable first — CANARY-002 FAILED section above, surviving `job-2/3/4.{json,log,reports}`, DB observations (2 members, 6 rows), journal — and the arming manifest archived. Confirmed disarm removed the two c5-j1 unit files + `manifest.json` only. **Post-state:** no horizon units loaded or on disk; cohort-5 evidence intact; cohorts 4 (1 member/0 obs, unarmed), 5 (2 members/6 obs, still failed — not rearmed), 6 (1 member/4 obs) untouched; zero provider calls; Alembic `0027`.

## CRYPTO-HORIZON-SHARED-CANDIDATE-FEASIBILITY-001 — local feasibility report dark-deployed (2026-07-16)

Measurement-only CLI `crypto-horizon-shared-candidate-feasibility-report` (`--hours/--days/--limit/--arm-margin-seconds/--format`); zero calls, zero writes, no migration (Alembic `0027`). Reuses the deployed `_completeness_reason`/`_horizon_windows`/`_shared_windows`/`ACTIVATION_GRACE` so it can't drift from the selector. See `docs/CRYPTO_HORIZON_SHARED_CANDIDATE_FEASIBILITY_2026_07_16.md` for the full findings.

**Verdict: CURRENT DISCOVERY SOURCE IS THE BLOCKER.** From ~4.37 days of local history (508 anchors, 7d/14d/30d ranges DATA-LIMITED): 198 complete-state (39%; only completeness gap is initial liquidity — pumpfun 29% complete vs graduated pools 100%), but complete tokens are persisted a **median ~85 min after their `first_evidence_at`** (the DexScreener profile/boost feed surfaces tokens well after birth; `created_at` is batch-stamped at scan time), so only **44/198 (8.7%)** are persisted while 15m-feasible. Birth timing is abundant — **197 complete pairs overlap in 15m windows** — but only **~6 genuine multi-minute-margin usable moments across 4 days (~1.4/day)**, caught historically only by scan-cadence luck (the Gate B attempt failed because no moment was live). CANARY-004 is **not reliably feasible** without a readiness detector or a fresher source; completeness/anchor were not relaxed.

**Recommended next milestone: CRYPTO-HORIZON-CANDIDATE-READINESS-001** — a measurement-only live readiness signal over persisted data that fires when two complete, still-15m-feasible tokens co-exist with adequate arm slack, removing the timing-luck dependency; its moment-rate data will scope any later `CRYPTO-DISCOVERY-FRESHNESS-001`.

**Dark validation (EVO-X2):** CLI `--help` OK; report on the real local DB reproduces the funnel/shared-window numbers with zero external calls and zero writes; 22 new mocked tests pass (full suite **1819 passed**); safety grep / AST / no-network audits clean; no cohort, observation, or unit created; Alembic `0027`. **Rollback:** `git reset --hard 1ae1fc2`.

## CRYPTO-HORIZON-CANDIDATE-READINESS-001 — candidate readiness evaluator + isolated MarketOps hook dark-deployed (2026-07-16)

Measurement-only readiness evaluator that detects when local persisted data holds an exact two-token shared-pass candidate set an operator could manually turn into CANARY-004. Composes the deployed feasibility calculations (`_completeness_reason`/`fifteen_window`/`pair_feasibility`/`safe_arm_deadline`/`ACTIVATION_GRACE`) — no rule duplication. Full doc: `docs/CRYPTO_HORIZON_CANDIDATE_READINESS.md`. No migration (Alembic `0027`). Commits `9ab3e9b` (impl+tests), `a179be5` (docs), `fe51ded` (expired-precedence fix).

**Readiness states (7, distinct):** `no_complete_candidates`, `no_overlapping_pair`, `pair_detected_not_due`, `pair_ready_for_manual_preparation`, `shared_due_now_ready`, `insufficient_arm_slack`, `expired`. Operator margin = deployed 45s grace + documented `OPERATOR_PREP_MARGIN_SECONDS=180` (five manual steps; not an env flag; override via `--minimum-arm-margin-seconds`). Deterministic ordering: greatest safe slack → earliest close → canonical id.

**CLIs (read-only, zero calls, zero writes, `persisted=false`):** `crypto-horizon-candidate-readiness-report` (`--at/--limit/--require-complete/--minimum-arm-margin-seconds/--format`) and `crypto-horizon-candidate-readiness-history-report`. `--require-complete` cannot be relaxed for the live verdict. Ready pairs print an operator-review **proposal** — the `--dry-run` selection only, never `--confirm`/arm, never executed.

**MarketOps hook (`MARKETOPS_INCLUDE_CANDIDATE_READINESS`, default false):** isolated, non-blocking, report-only, runs AFTER the crypto persistence stage; no new timer/daemon, no second scan; cannot fail the cycle (exception caught, never re-raised even under `fail_fast`, recorded in `run.summary["candidate_readiness_error"]`). Off = complete no-op. When on, appends one secret-free line/cycle to `~/crypto-horizon-readiness/readiness.jsonl` (runtime path, never committed).

**Dark validation (EVO-X2, zero provider calls):** live report → `status=expired` (508 candidates, 198 complete, 197 overlapping, 0 feasible-15m/usable now); at the historical `2026-07-12T05:35Z` moment → `shared_due_now_ready` correctly detecting BREAD+ギコ with a proposed dry-run (validates end-to-end against the feasibility data); history report → 0 evaluations (empty audit). CLI `--help` OK; 32 mocked tests pass (full suite **1851 passed, 2 skipped**); safety grep / AST / no-network audits clean. **Baseline unchanged:** Alembic `0027`; **0 horizon units**; cohorts 4 (1/0), 5 (2/6), 6 (1/4) untouched; `readiness.jsonl` absent and flag `False` — **deploying the code did not activate the hook or write any data**; `.env`/flags/MarketOps behavior unchanged. **Rollback:** `git reset --hard 2fd3126`.

**Activation gate:** the hook is **READY FOR MEASUREMENT ACTIVATION** — provably non-blocking/report-only and validated dark — but activation is gated on an explicit human flip of `MARKETOPS_INCLUDE_CANDIDATE_READINESS=true` (not performed here; no `.env`/flag change made). Deploying the code alone does not activate it. Recommended observation period once activated: **14 days** (7-day interim checkpoint) to accumulate ~20 usable moments at the measured ~1.4/day rate and estimate whether the ~6-min MarketOps cadence reliably lands a cycle inside each fleeting (~13-min window, few-min actionable slack) moment.

## CRYPTO-HORIZON-CANDIDATE-READINESS-001 — measurement activation (2026-07-16, human-authorized): PASS

Human-authorized activation of the already-dark-deployed, measurement-only readiness hook. Exactly one flag changed on EVO-X2; nothing else.

- **Activation (UTC / America/Los_Angeles):** flag edited ~2026-07-16T19:55:56Z / 2026-07-16T12:55:56-07:00; first flag-on cycle **3097** started 2026-07-16T19:55:57Z / 12:55:57-07:00, readiness evaluated 2026-07-16T19:56:26Z / 12:56:26-07:00.
- **Synchronized code commit:** `d765141` (Mac = origin = EVO-X2).
- **Authorized change (only):** `MARKETOPS_INCLUDE_CANDIDATE_READINESS=true` appended to EVO-X2 `~/projects/probability-arena/.env` (154→157 lines: one comment + the flag; sha `8517188e…`→`5f70c0ae…`; no other line touched, no secret exposed, `.env` not committed). No service restart — the MarketOps oneshot timer re-reads `.env` (`EnvironmentFiles`) each run. No cadence/schedule/timer/daemon change. Resolved `include_candidate_readiness` False→True.
- **Pre/post MarketOps cycle IDs:** last pre-activation cycle **3096** (ok); first activated cycle **3097** (ok, 19:55:57→19:57:01Z, all stages ok, `stage_errors={}`).
- **First live readiness record** (`~/crypto-horizon-readiness/readiness.jsonl`, runtime path, never committed): exactly **1 line** attributable to cycle 3097 — `run_id=3097`, `marketops_cycle_id=3097`, `evaluated_at=2026-07-16T19:56:26Z`, `state=expired`, `complete_candidates=198`, `overlapping_pairs=197`, `usable_pairs=0`, `candidate_pair=null`, `rejection_reason=expired`, `external_calls=0`. Secret-free; one line per distinct cycle.
- **Readiness-attributable external calls = 0** (`summary.candidate_readiness.external_calls=0` and JSONL `external_calls=0`). **No second scan:** exactly one crypto watcher run (id 3096) in the cycle window. The global SolanaTracker counter moved +15 (hour/today/month; rolling_24h +0) = one normal crypto-stage run's `PER_RUN_LOOKUP_LIMIT=15` via `ENABLE_CRYPTO_RISK_ENGINE` — the existing crypto stage, NOT the hook.
- **No cohort, observation, arming, unit, timer, or daemon:** cohorts still 1–6 (4:1/0, 5:2/6, 6:1/4 unchanged); zero horizon units; Alembic `0027`; no MarketOps schedule or provider-policy change; `candidate_readiness_error=None`.
...
## CRYPTO-HORIZON-CANDIDATE-READINESS-001 — Activation Sanity Check (recorded 2026-07-16)

`ACTIVATION SANITY CHECK: PASS` · `SEVEN-DAY CHECKPOINT: NOT YET DUE` · `RECOMMENDATION: CONTINUE MEASUREMENT UNCHANGED`. Recorded at 2026-07-16T20:02Z — **~6.5 min / 2 cycles after activation** — an activation sanity check that the hook behaves correctly immediately post-activation. It is **not** a seven-day checkpoint/sample/measurement (the nominal 7-day point 2026-07-23 has not elapsed). Full detail: `docs/CRYPTO_HORIZON_READINESS_ACTIVATION_SANITY_2026_07_16.md`. (Supersedes the earlier "7-day interim checkpoint" label for this same two-cycle snapshot.)

**Hook reliability: PASS.** 2 records (cycles 3097, 3098), both `expired`, 1 record/cycle, no duplicates, `external_calls=0`, secret-free, schema-valid, append-only. MarketOps: 2/2 cycles ok, 0 failures, 0 evaluator errors, exactly 1 crypto scan/cycle (no second scan), readiness-attributable provider calls/cohorts/observations/units = 0; cohorts 1–6 unchanged; Alembic `0027`; no auxiliary readiness timer/daemon (only long host `sleep`s are unrelated CUPS/backup daemons). No usable historical moment existed in the ~6.5-min window (0 fresh complete-in-window births; consistent with the ~85-min discovery lag, not a hook fault); no catch-rate claim is made. JSONL ~370 B/record → ~86 KB/day (≈1.17 MB/14 d, ≈2.51 MB/30 d), bounded.

**Recommendation: CONTINUE MEASUREMENT UNCHANGED** — reliability sound, two-cycle snapshot only, no actionable pair. Real seven-day checkpoint **2026-07-23 after 19:56 UTC / 12:56 PM PT**; fourteen-day **2026-07-30 after 19:56 UTC / 12:56 PM PT**. No engineering/operational action before the actual seven-day checkpoint unless the hook causes an operational failure. No cohort/arming occurred.

## CRYPTO-HORIZON-CANDIDATE-READINESS-001 — Real Seven-Day Checkpoint + controlled resynchronization (2026-07-24, ~02:00–02:33 UTC)

`SEVEN-DAY CHECKPOINT: PASS WITH OPERATIONAL FINDINGS` · `RECOMMENDATION: CONTINUE MEASUREMENT TO JULY 30`. Full analysis: `docs/CRYPTO_HORIZON_READINESS_CHECKPOINT_7D_2026_07_23.md` (interval 2026-07-16T19:56:26Z → 2026-07-24T02:02:55Z; 1,751/1,751 cycles recorded, integrity perfect, zero readiness-attributable calls/writes/units/failures; catch rate vacuous — birth-anchor lane starved while raw discovery persisted 2,079 tokens incl. ~15 estimated usable pairs; DB 3.68 GB past the 3,072 MB absolute gate; 51 lock events, none readiness-attributable). No cohort, arming, scan, or provider call from the checkpoint itself.

**Resynchronization (Gate B):**
- **Commits:** `3f742c9 → e7a2753` via `git fetch` + `git pull --ff-only origin main` (~02:13 UTC). Post-pull: Mac = origin = EVO-X2 = `e7a2753`.
- **Diff audited pre-pull:** 5 commits, 9 paths — docs only (`docs/CAPABILITY_MATRIX.md`, `PROJECT_CANON.md`, `ROADMAP.md`, `SAFETY_BOUNDARIES.md`, + 4 new design/checkpoint docs) plus the previously reviewed **inert `app/canon.py` constants** (string tuples; imported only by the lazy read-only `agent-context` handler — outside the MarketOps runtime path). No forecast-PR code, no MarketOps/readiness/crypto-runtime/provider/DB/migration/systemd/config/model/pragma change.
- **Alembic:** `0027` before and after (no migration in diff). **Flags:** unchanged (`.env` untouched; `MARKETOPS_INCLUDE_CANDIDATE_READINESS=true` remains). **External calls:** 0. **MarketOps:** not restarted.
- **Post-sync natural cycle:** #4851 (started 02:32:03 UTC, first cycle fully after the pull, naturally scheduled): status `ok`, exit 0, 41.9 s, `stage_errors={}`, exactly one crypto scan, exactly one readiness record appended (cycle 4851, `expired`, `external_calls=0`), cohorts 6 / observations 35 unchanged, 0 horizon units. No runtime behavior change.
- **Rollback (if ever needed):** `git reset --hard 3f742c9` (docs + inert constants only; no data/service dependency).
- Note: a pre-existing 250-byte untracked command-typo artifact (filename beginning `ystemctl --user list-timers…`, dated 2026-07-15) remains in the EVO repo dir, untouched — flagged for manual cleanup.

## SQLITE-LOCK-TELEMETRY-001A — nonblocking writer telemetry dark-deployed (2026-07-24, ~04:14 UTC)

First implementation slice of `docs/SQLITE_LOCK_TELEMETRY_DESIGN_2026_07.md`, executed after the 07-23 checkpoint per the design's slice gating. Details: `docs/SQLITE_LOCK_TELEMETRY_001A.md`.

- **Commits:** `e7a2753 → 0385d53` (`63a72cc` primitives + `0385d53` tick-aggregation/backup instrumentation) via `git pull --ff-only` (~04:14 UTC). MarketOps **not restarted**; `.env`, timers, units, pragmas, schedules untouched. **Alembic `0027`** before and after (no migration).
- **Validation (Mac):** full suite **1882 passed** incl. 31 new telemetry tests (sink atomicity/concurrency/permissions, failure isolation, parity, two-connection real lock-contention integration test, emit p99 < 1 ms); safety grep + AST audit clean; three independent reviews (SQLite correctness/parity: APPROVE; security: pass with hardening applied — `O_NOFOLLOW`, 0700 enforcement, listener guards; observability architecture: design-faithful, deviations documented).
- **Dark proof on EVO:** after the pull, `~/probability-arena-telemetry/` did **not** exist through MarketOps cycles #4868–#4874 (hot path uninstrumented — deploy is dark). It was created **only** by the naturally scheduled tick-aggregation run #358 (04:53 UTC): dir `0700`, file `0600`, **16 events, 0 malformed, 0 secret-pattern hits** — 15 `commit_unit` children (commit_ms 0–16 ms exact; per-sub-window hold ≈ 310 ms instrumented estimate; retries 0) all `parent_event_id`-correlated to 1 `aggregate` parent (rows_read 109,050, buckets 23,376, `database_bytes` 3,681,239,040 gauge). Run #358 itself: `ok`, 13 sub-windows, 0 failed windows, 0 retries — writer behavior unchanged.
- **Backup instrumentation** validated on EVO via a fully disposable temp-scoped check (temp DB + temp `SQLITE_TELEMETRY_DIR`; no real backup, no real-DB contact): `BackupResult` + `verify_backup ok`, 1 schema-valid secret-free event with `journal_mode=delete`/`synchronous=2` samples and gauges; temp dirs removed.
- **Isolation:** **0** `database is locked` events since the deploy; MarketOps cycles #4868/#4875 `ok` with exactly one readiness record each (`external_calls=0`, JSONL at 1,779 lines and growing normally); cohorts/observations/units unchanged; **external calls 0** (telemetry has no network capability, statically enforced by test).
- **Rollback:** `git revert 0385d53 63a72cc` removes the package and both emit sites with zero residue; `~/probability-arena-telemetry/` is disposable (delete at will; never backed up).
- **Not in this slice:** MarketOps/watcher/crypto/meme/horizon instrumentation, shared-engine listeners, report commands, rotation, alerts (001B–001E; hot-path slices additionally wait for the readiness window close 2026-07-30).

## CRYPTO-HORIZON-ANCHOR-FEED-MEASUREMENT-001 — exact-cycle anchor feed dark-deployed + ACTIVATED (2026-07-24, ~07:45–07:52 UTC)

Details: `docs/CRYPTO_HORIZON_ANCHOR_FEED_MEASUREMENT.md`. Measurement only; provider-free structurally; never a trading surface.

- **Commits:** `8c9d536 → 5b5307f` (`4383172` exact-cycle service + CLI, `98f9561` isolated MarketOps hook + flag + 27 tests, `5b5307f` docs) via ff-only pull (~07:45 UTC). Alembic `0027` (no migration). Full suite 1,909 passed; three independent reviews (correctness/duplication, security/provider-impossibility, transaction-safety/contract) — both confirmed HIGH findings fixed pre-merge (transitive adapter import removed via `_completeness_reason` relocation; spike-cycle self-lock removed via flag-gated checkpoint commit) with regression tests.
- **Dark cycle #4904** (07:45:35, flag absent): `ok`, exit 0, **no `anchor_feed` key** (disabled hook = complete no-op), one scan, one readiness record (line 1,808), anchors 511 unchanged, cohorts 6 / obs 35 / units 0.
- **ACTIVATION (authorized): 2026-07-24T07:46:20Z** — exactly one line appended to EVO `.env`: `MARKETOPS_INCLUDE_CRYPTO_TAPE_ANCHOR_FEED=true` (sha256 `5f70c0ae… → 985640b5…`, diff = that line only; not committed; no restart — the oneshot re-reads `.env`). **Epoch 4 begins at the next natural cycle.**
- **First live cycle #4905** (07:51:03→07:51:38, natural, `ok`, exit 0): a NEW-TOKEN cycle — `anchor_feed{status=ok, source_crypto_run_id=4904, tokens_received=2, tokens_validated=2, anchors_created=2, anchors_existing=0, complete_anchors=1, incomplete_anchors=1, skipped_cap=0, external_calls=0, duration_ms=234}`. Anchors 511 → **513** (latest 07:51:21 — materialized ~18 s after token persistence vs the historical ~85 min lag); readiness evaluated the new anchors **in the same cycle** (`complete_candidates` 200 → 201, state `expired` — only one complete anchor arrived this cycle, correctly no pair), `external_calls=0`, exactly one scan, one readiness record (1,809). Cohorts/observations/units 6/35/0 unchanged; DB bytes unchanged (3,681,239,040); ~74.4 GB free; 0 lock events 07:45→07:52; provider counters normal cadence only; telemetry file movement is the scheduled tick-aggregation writer only.
- **Operating rules through July 30 (Epoch 4):** hook stays enabled; no extra scans, no manual tape runs, no cohort creation, no arming, no observation triggering, no cadence/completeness/anchor change; readiness never interpreted as market opportunity. If a live pair appears: preserve evidence, request separate CANARY-004 approval — nothing creates or arms automatically. July-30 must report Epochs 1–4 separately (Epoch 4 catch-rate never combined with Epoch 1's starved records).
- **Rollback:** remove the `.env` line (hook = no-op next cycle) and/or `git revert 98f9561 4383172`.

## CRYPTO-HORIZON-CANDIDATE-READINESS-001 — 14-day checkpoint + controlled resynchronization (2026-07-31, ~20:50–21:05 UTC)

Closes the readiness observation window opened at activation. Details: `docs/CRYPTO_HORIZON_READINESS_CHECKPOINT_14D_2026_07_30.md`. Analysis + docs + inert constants only; no cohort created or armed, no scan, tape run, provider call, observation, timer, unit, or DB write produced by this work.

- **Checkpoint verdict: PASS; window CLOSED.** Interval `2026-07-16T19:56:26Z → 2026-07-31T20:43:20.9Z` (15.033 d), 3,625 records, cycles 3097–6721 contiguous. Coverage exact and bidirectional: 0 missing ids, 0 MarketOps cycles without a readiness record, 0 records without a run. Integrity: 0 invalid JSON, 0 schema mismatch, 0 secret hits, 0 `external_calls` violations, 0 error states. Ran ~24.8 h past the nominal threshold because an operator-workstation shutdown interrupted the checkpoint session mid-analysis — the host-side measurement was unaffected and lost no cycles.
- **Epochs reported separately (required):** E1 cycles 3097–4881 n=1,785 100% `expired` / **0 live**; E2 cycles 4882–4885 n=4 (King+Octen); E3 cycles 4886–4904 n=19 `expired`; **E4 cycles 4905–6721 n=1,817**.
- **Epoch-4 safety contract held on every invariant:** 1,817 cycles 100% `ok`, zero stage and anchor-feed errors, `external_calls=0`, `skipped_cap=0`, exactly 1 scan/cycle, 1,817 distinct `source_crypto_run_id` (zero reuse), 1,422 tape runs **all `mode=exact_cycle`, zero manual**, and **0 cohorts / 0 members / 0 observations created** (all-time still 6 / 19 / 35, unchanged since CANARY-003). Hook cost median 171 ms against a 36.8 s cycle; E4-vs-E1 cycle duration +1.6 s (+4.5%), p95 unchanged.
- **Primary 7-day finding RESOLVED:** birth-anchor lag ~85 min → **median 17.2 s** (p95 35.9 s); **2,773/2,773 = 100%** of anchors persisted while 15m-feasible (vs 8.7%); live moments **0 in 1,785 cycles → 496 distinct pairs in 1,817 cycles** (~66/day, stable across all 8 days). Median safe slack 377 s; **62.1% of live episodes persist across ≥2 consecutive cycles** — the binding constraint is now human approval timing, not pair scarcity.
- **Resynchronization:** commits `6ac5503 → 4e21949` via `git fetch` + `git pull --ff-only origin main` (~20:56 UTC), then `4e21949 → <canon>` for the canon refresh. Post-pull Mac = origin = EVO-X2. **Diff audited pre-pull:** 2 commits, 2 paths — one new checkpoint doc + `tests/test_crypto_horizon_cohort_select_001.py`. No MarketOps/readiness/crypto-runtime/provider/DB/migration/systemd/config change; nothing on the MarketOps import closure.
- **Alembic `0027`** before and after (no migration). **Flags unchanged** (`.env` untouched; `MARKETOPS_INCLUDE_CANDIDATE_READINESS=true` and `MARKETOPS_INCLUDE_CRYPTO_TAPE_ANCHOR_FEED=true` remain). **MarketOps not restarted.**
- **Post-sync validation:** pulled tests re-run on EVO (39 passed, in-memory only, no prod DB contact). First fully post-pull natural cycle **#6724** (21:01:03→21:01:40): `ok`, 36.4 s, `stage_errors={}`, `anchor_feed{status=ok, external_calls=0, error=null}`, exactly one readiness record (`pair_ready_for_manual_preparation`, `external_calls=0`), cohorts 6 / obs 35 / units 0 unchanged. No runtime behavior change.
- **Canon refresh (`app/canon.py`, inert constants — lazily imported only by the read-only `agent-context` handler, outside the MarketOps runtime path):** `agent-context` had gone stale, still describing both checkpoints as upcoming and naming the discovery source as the blocker. Now states the window is closed and both checkpoints passed; that CANARY-004 is the recommended next step but still needs explicit approval **at** a live moment; that CRYPTO-DISCOVERY-FRESHNESS-001 is **superseded** (its premise was overturned); that the storage gate wants its own OPS milestone; and that the freeze-deferred merges are eligible.
- **Test time-bomb fixed en route:** `tests/test_crypto_horizon_cohort_select_001.py` pinned an absolute `NOW = 2026-07-15T12:00Z` against a `hours=240` lookback, and `create_cohort` takes no `now` parameter (it reads the real clock), so 4 tests had been failing since 2026-07-25 for calendar reasons alone. Product code unaffected; other absolute-date test anchors are safe because they inject `now=` explicitly.
- **Standing observations (unchanged, pre-existing, no action taken):** the app-level 3072 MiB DB gate is exceeded at ~132% (4.25 GB; 295 open critical `db_growth_warning`, first fired 2026-07-05, growth dominated by `market_price_ticks` ~2.1 GB) while the host itself is fine (62% used, 88 G free) — E4 growth 75.9 MB/day, **down** from ~115; `probability-arena-backup.timer` is listed in canon as expected but is still **not installed** (pre-existing R7); the expired one-shot `provider-roll-001-t24h.timer` from 2026-07-08 is still loaded but will never fire again; and the 2026-07-15 untracked command-typo artifact remains in the EVO repo dir, untouched.
- **Rollback:** `git reset --hard 6ac5503` (docs, tests, and inert constants only; no data or service dependency).

## SQLITE-BACKUP-COORDINATION-001 — verified backups deployed + timer ACTIVATED (2026-08-03, ~00:02–00:15 UTC)

Closes the backup gap found by live inspection on 2026-08-02: the timer had **never been installed** (`is-enabled` → `not-found`, 0 unit files) and the newest backup was **23 days stale** against a 4.19 GiB database. Design and restore runbook: `docs/SQLITE_BACKUP_COORDINATION_001.md`.

- **Commits:** `8adf5e8 → 17221d8` (implementation + docs + two review passes) then `→ 4229c00` (third-review durability fixes) via audited ff-only pulls. Alembic `0027` before and after (no migration). MarketOps **not restarted**. Both `MARKETOPS_INCLUDE_*` flags untouched.
- **Config:** one reversible line appended to EVO `.env` — `BACKUP_DIR=/mnt/data/probability-arena-backups` (136 → 137 lines, sha `985640b5… → 7c1b3884…`). Destination is a **separate ext4 volume** (712 GiB free, 5% used, in `/etc/fstab`) — deliberately off the root volume that holds the database at 61%. Directory `0700`, artifacts and manifests `0600`.
- **Two constraints confirmed on the live host before shipping:** `journal_mode=delete` + `busy_timeout=30000` (a single-step copy of 1,090,997 pages would hold one read lock past the busy handler and turn recoverable waits into hard lock failures), and `/tmp` on the **root volume** (verification would decompress 4.19 GiB onto the volume being protected). Fixed by a chunked copy and a pinned `TMPDIR`; WAL and pragma changes were out of scope and were not made.
- **Dark validation:** `systemd-analyze verify` on rendered units → **rc=0**; `backup-db --dry-run` → rc=0 and **created nothing** (artifact count unchanged).
- **One measured production backup (Gate 18), 00:02:15Z:** `status=success exit 0`, **37 s wall clock** — copy 24,557 ms, verification 11,279 ms, **`online_copy_restarts=0`**. Artifact `backup-20260803T000215Z.db.gz`, 450,543,730 B (429.67 MiB) from a 4,468,723,712 B source; sha256 `4926c3ad…` matching the manifest; `integrity_check=ok`, 53 tables, Alembic `0027` read from the backup; counts markets 95,910 / market_forecasts 12,132 / marketops_runs 7,236. Independent `verify-db-backup` → **OK**. This empirically disproved the third review's H1 (chunked-copy convergence) blocker.
- **Isolation across the backup window:** database-locked events **4 before, 4 after — unchanged**; telemetry +1 line (exactly the one backup event: `outcome=success external_calls=0 journal_mode=delete`); MarketOps cycle 7237 ran **during** the window and completed `ok` with `stage_errors={}` and both hooks at `external_calls=0`; root-volume free space essentially unchanged (the artifact landed on `/mnt/data`); cohorts/members/observations untouched.
- **Timer installed + enabled (Gate 19), 00:14Z:** only the two backup units copied; the unit inventory **excluding** them is md5-identical (`147d2c40b9fdcb089bd96b92c9aeb11a`) before and after — no unrelated unit changed. `Linger=yes` confirmed, so the user timer survives logout. Timer `loaded/active/waiting`, `Persistent=yes`, `LastTriggerUSec` **empty**, next elapse **2026-08-03 01:34:22 UTC (18:34:22 PDT)**. Service `inactive/dead` — **no immediate backup was triggered** (artifact count still 1). 7 probability-arena timers, 0 failed units.
- **Status: ACTIVE, FIRST NATURAL RUN PENDING.** The first scheduled backup executes host-owned at 01:34 UTC; capture it from durable evidence in a later session.
- **Rollback:** `systemctl --user disable --now probability-arena-backup.timer && rm ~/.config/systemd/user/probability-arena-backup.{service,timer} && systemctl --user daemon-reload`, and remove the `BACKUP_DIR` line from `.env`. Existing artifacts are inert and are never deleted by uninstalling.

## Live ANALYZE — planner-maintenance state, recorded (CRYPTO-BACKLOG-SELECTION-AND-OPERATOR-PATH-001 B0, 2026-08-11)

**This is production truth that was live on EVO-X2 and recorded nowhere on `main` before this entry.** `AGENTS.md` requires the runbook and this report to be updated when EVO state changes; `grep "ANALYZE|sqlite_stat"` over `docs/EVO_X2_RUNBOOK.md`, this file, and `DEPLOYMENT_AUDIT_EVO_X2.md` returned zero hits until now.

- **What happened:** `ANALYZE` was run against the live EVO-X2 database as an **operator-controlled maintenance action**, executed **2026-08-11T07:04:51Z**. It is **not** a migration, a schema change, or anything Alembic tracks — SQLite's query planner statistics table (`sqlite_stat1`) is separate from the schema, and nothing in this codebase runs `ANALYZE` automatically today.
- **State before → after:** `sqlite_stat1` **absent → 130 rows**. Wall clock **~0.46 s**. **DB byte size delta: 0** (statistics tables are tiny; this is not a storage event). `PRAGMA integrity_check` → `ok`, both before and after. Alembic revision **unchanged at `0027`** throughout. **Zero new lock events** attributable to the run, confirmed against **20+ clean MarketOps cycles** observed since.
- **What this changes:** query planner statistics now **materially affect reconciliation query-plan selection** on this database (see `docs/CRYPTO_QUERY_PLAN_AND_DENOMINATOR_RECOVERY_001.md` for the plan-sensitivity mechanism this was measuring). This is **planner-maintenance state, not schema/migration state** — stated plainly because it is easy to conflate the two: `alembic current` will never show that `ANALYZE` has or hasn't run, and no migration depends on it existing, EXCEPT where noted below.
- **No recurring `ANALYZE` timer exists.** This was a one-off operator action, not an installed unit. `systemctl --user list-timers` on EVO-X2 shows 7 probability-arena timers and none of them run `ANALYZE`. If and when a recurring cadence is proposed, it needs its own milestone and approval — this entry documents only what already happened.
- **Backup interaction:** every backup artifact taken **before** 2026-08-11T07:04:51Z was captured from an **UNANALYSED** database (no `sqlite_stat1` row at all). Restoring any of those backups restores that unanalysed planner state, silently, even though the schema and data are otherwise fully intact — see `docs/EVO_X2_RUNBOOK.md`'s restore-verification step (B6) for the explicit post-restore planner check this now requires.
- **Migration 0028 interaction:** migration `0028` (`crypto_token_lifecycle_run_status_width`) rebuilds `crypto_token_lifecycle_runs` via `batch_alter_table`, and SQLite's table-rebuild machinery does not preserve that table's `sqlite_stat1` row across the rebuild — **measured 130 → 129 rows** applying `0028` to a production copy. See `docs/EVO_X2_RUNBOOK.md`'s "0028 rollback procedure" (B8) and the migration's own post-upgrade statistics step (B5) for how this is handled going forward.
- **Verified EVO state at the time of this entry (2026-08-11):** `main` at `2c8f75b`, Alembic `0027`, MarketOps `60/60 ok`, watcher active, `sqlite_stat1` = 130 rows, DB `4,550,623,232` bytes / 36.1% freelist, latest backup `backup-20260811T013803Z`, 7 timers (no crypto timer), arrivals ~494/day. This branch (`worktree/crypto-coverage-repair`) makes no further live changes to EVO — see its own commit history for what is implemented-but-not-deployed.
